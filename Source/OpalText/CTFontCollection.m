...
- (id)initWithFontDescriptors: (NSArray*)descriptors options:
  (NSDictionary*)opts
{
  self = [super init];
  if (nil == self)
...
- (CTFontCollection*)collectionByAddingFontDescriptors: (NSArray*)descriptors
                                               options: (NSDictionary*)opts
{
  NSArray *newDescriptors = [_descriptors arrayByAddingObjectsFromArray:
                                          descriptors];
  CTFontCollection *collection = [[CTFontCollection alloc]
                                    initWithFontDescriptors: newDescriptors
                                  options: opts];
...
- (NSArray*)fontDescriptorsSortedWithCallback:
  (CTFontCollectionSortDescriptorsCallback)cb
                                         info: (void*)info
{
  return [_descriptors sortedArrayUsingFunction: (NSComparisonResult (*)(id, id,
                       void*))cb
                                        context: info];
}
...
{
  return [[base collectionByAddingFontDescriptors: descriptors options: opts]
          retain];
}
...
CTFontCollectionRef CTFontCollectionCreateWithFontDescriptors(
  ...
  CFDictionaryRef opts)
{
  return [[CTFontCollection alloc] initWithFontDescriptors: descriptors options:
                                   opts];
}
