  ...
  if (!isCairoDrawingIntoUserBuffer)
    {
      ...
      const size_t srcBytesPerRow = cairo_format_stride_for_width(
                                      cairo_image_surface_get_format(srcCairoSurface), srcWidth);
      const CGBitmapInfo srcBitmapInfo = kCGBitmapByteOrder32Host |
  ...
  void *releaseInfo)
{
  ...
  const BOOL nativeCairoSupport = isFormatNativelySupportedByCairo(info, cs,
                                  bitsPerComponent, width, bytesPerRow, &format);
  if (nativeCairoSupport)
    {
      // Cairo can draw directly into the buffer format the caller provided or requested
      surf = cairo_image_surface_create_for_data(data, format, width, height,
             bytesPerRow);
  ...
  else
    {
      ...
      surf = cairo_image_surface_create_for_data(cairoData, format, width, height,
             cairoBytesPerRow);
...
size_t CGBitmapContextGetBitsPerPixel(CGContextRef ctx)
{
  if ([ctx isKindOfClass: [CGBitmapContext class]])
    {
      size_t colorComps = CGColorSpaceGetNumberOfComponents(((
                            CGBitmapContext *)ctx)->cs);
      size_t alphaComps = 0;
