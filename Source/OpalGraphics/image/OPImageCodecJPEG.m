...
static void gs_jpeg_error_mgr_create(j_common_ptr cinfo,
                                     gs_jpeg_error_mgr_ptr errMgr)
{
...
static boolean gs_fill_input_buffer(j_decompress_ptr cinfo)
{
  ...
  src->parent.bytes_in_buffer = OPDataProviderGetBytes(src->dp, src->buffer,
                                OPAL_BUFFER_SIZE);
 ...
 * has to be called.  */
static void gs_jpeg_memory_src_create(j_decompress_ptr cinfo,
                                      CGDataProviderRef dp)
{
...
static void gs_term_destination (j_compress_ptr cinfo)
{
  ...
  OPDataConsumerPutBytes(dest->dc, dest->buffer,
                         OPAL_BUFFER_SIZE - dest->pub.free_in_buffer);
}

static void gs_jpeg_CGDataConsumer_dest_create (j_compress_ptr cinfo,
    CGDataConsumerRef dc)
{
...
- (CGImageRef)createImageAtIndex: (size_t)index options: (NSDictionary*)opts
{
  ...
  NS_DURING
  {
    ...
    NSData *imgData = [[NSData alloc] initWithBytesNoCopy: imgbuffer length:
                                      cinfo.output_height * rowSize freeWhenDone: YES];
...
- (bool) finalize
{
  ...
  NS_DURING
  {
    ...
    cinfo.input_components = CGColorSpaceGetNumberOfComponents(
    ...
    NSNumber *qualityNumber = [(NSDictionary*)props objectForKey:
    ...
    NSNumber *progressiveNumber = [(NSDictionary*)props objectForKey:
                                                        @"NSImageProgressive"];
