...
void OPImageFormatLog(OPImageFormat fmt, NSString *msg)
{
  ...
  NSDebugLLog(@"Opal",
              @"%@: <%@, color components=%d, alpha?=%d, premul?=%d, alpha last?=%d>",
              msg, compFormatString, fmt.colorComponents, fmt.hasAlpha,
              fmt.isAlphaPremultiplied, fmt.isAlphaLast);
}
  ...
  if (msb_off % 32 == 0 && bit_width == 32)
  ...
  else
    {
      ...
      switch (bytes_needed)
        {
          case 5:
            chars[4] = base[byte1+4];
          case 4:
            chars[3] = base[byte1+3];
          case 3:
            chars[2] = base[byte1+2];
          case 2:
            chars[1] = base[byte1+1];
          case 1:
            chars[0] = base[byte1+0];
            break;
          default:
            abort();
        }
 ...
 */
static void OPRoundUp(size_t bitsPerComponentIn, size_t bitsPerPixelIn,
                      size_t *bitsPerComponentOut, size_t *bitsPerPixelOut)
{
  ...
  *bitsPerPixelOut = (bitsPerPixelIn / bitsPerComponentIn) *
                     (*bitsPerComponentOut);
}
  ...
  OPImageFormat *out)
{
  ...
  out->hasAlpha = (alpha != kCGImageAlphaNone
                   && actualComponents > colorComponents);
  ...
  CGColorRenderingIntent intent)
{
  ...
  if (!OPImageFormatForCGFormat(srcBitsPerComponent, srcBitsPerPixel,
                                srcBitmapInfo, srcColorSpace, &srcFormat))
    {
      NSLog(@"Input format not supported");
    }
  if (!OPImageFormatForCGFormat(dstBitsPerComponent, dstBitsPerPixel,
                                dstBitmapInfo, dstColorSpace, &dstFormat))
    {
