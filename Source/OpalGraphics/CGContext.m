...
static inline void set_color(cairo_pattern_t **cp, CGColorRef clr,
                             double alpha);
...
{
  OPLOGCALL("ctx /*%p*/, CGAffineTransformMake(%g, %g, %g, %g, %g, %g)", ctx,
            transform.a,
...
CGAffineTransform CGContextGetCTM(CGContextRef ctx)
{
  ...
  OPRESTORELOGGING()
  return CGAffineTransformMake(result.xx, result.yx, result.xy, result.yy,
                               result.x0, result.y0);
}
  ...
  size_t count)
{
  ...
  if (!lengths && count != 0)
    {
      NSLog(@"%s: null 'lengths' passed with count %d. Fixing by setting count to 0.",
            lengths, (int)count);
      count = 0;
    }
...
{
  OPLOGCALL("ctx /*%p*/, %s", ctx,
            shouldSubpixelPositionFonts ? "true" : "false")
  OPRESTORELOGGING()
}
...
{
  OPLOGCALL("ctx /*%p*/, %s", ctx,
            allowsFontSubpixelPositioning ? "true" : "false")
  OPRESTORELOGGING()
}
...
{
  OPLOGCALL("ctx /*%p*/, %s", ctx,
            shouldSubpixelQuantizeFonts ? "true" : "false")
  OPRESTORELOGGING()
}
...
{
  OPLOGCALL("ctx /*%p*/, %s", ctx,
            allowsFontSubpixelQuantization ? "true" : "false")
  OPRESTORELOGGING()
}
...
void CGContextSetShadow(
  ...
  CGFloat radius)
{
  OPLOGCALL("ctx /*%p*/, CGSizeMake(%g, %g), %g", ctx, offset.width,
            offset.height,
            radius)
...
void CGContextSetShadowWithColor(
  ...
  CGColorRef color)
{
  OPLOGCALL("ctx /*%p*/, CGSizeMake(%g, %g), %g, <color>", ctx, offset.width,
            offset.height,
            radius)
...
void CGContextAddCurveToPoint(
  ...
  CGFloat y)
{
  OPLOGCALL("ctx /*%p*/, %g, %g, %g, %g, %g, %g", ctx, cp1x, cp1y, cp2x, cp2y, x,
            y)
  ...
  CGFloat y)
{
  ...
  CGContextAddCurveToPoint(ctx, (curr.x/3.0) + (2.0*cpx/3.0),
                           (curr.y/3.0) + (2.0*cpy/3.0),
...
{
  OPLOGCALL("ctx /*%p*/, CGRectMake(%g, %g, %g, %g)", ctx, rect.origin.x,
            rect.origin.y,
...
void CGContextAddArc(
  ...
  int clockwise)
{
  OPLOGCALL("ctx /*%p*/, %g, %g, %g, %g, %g, %d", ctx, x, y, radius, startAngle,
            endAngle,
            clockwise)
  if (clockwise)
...
{
  OPLOGCALL("ctx /*%p*/, CGRectMake(%g, %g, %g, %g), %g", ctx, rect.origin.x,
            rect.origin.y,
                             ...
                             double alpha)
{
  ...
  CGColorRef srgbClr = OPColorGetTransformedToSpace(clr, srgb,
                       kCGRenderingIntentRelativeColorimetric);
  CGColorSpaceRelease(srgb);

  const CGFloat *cc = CGColorGetComponents(srgbClr);
  NSDebugLLog(@"Opal", @"Set color with %f %f %f %f", (float)cc[0], (float)cc[1],
              (float)cc[2], (float)cc[3]*alpha);
...
{
  OPLOGCALL("ctx /*%p*/, %g, %g, %g, %g, %g", ctx, cyan, magenta, yellow, black,
            alpha)
  CGColorRef color;
...
void opal_draw_surface_in_rect(CGContextRef ctxt, CGRect rect,
                               cairo_surface_t *src, CGRect srcRect)
{
...
void CGContextDrawImage(CGContextRef ctx, CGRect rect, CGImageRef image)
{
  ...
  OPLOGCALL("ctx /*%p*/, CGRectMake(%g, %g, %g, %g), <image>(%p, subrect %g %g %g %g)",
            ctx, rect.origin.x,
            rect.origin.y, rect.size.width, rect.size.height, image, srcrect.origin.x,
            srcrect.origin.y, srcrect.size.width, srcrect.size.height)
...
{
  OPLOGCALL("ctx /*%p*/, CGRectMake(%g, %g, %g, %g), <image>", ctx,
            rect.origin.x,
...
void CGContextDrawPDFDocument(
  ...
  int page)
{
  OPLOGCALL("ctx /*%p*/, CGRectMake(%g, %g, %g, %g), <image>, %d", ctx,
            rect.origin.x,
...
static void opal_AddStops(cairo_pattern_t *pat, CGGradientRef grad)
{
  ...
  size_t cs_numcomps =
    4; // == (CGColorSpaceGetNumberOfComponents(OPGradientGetColorSpace(grad)) + 1);
  ...
  for (int i=0; i<numcomps; i++)
    {
      cairo_pattern_add_color_stop_rgba(pat, locations[i], components[i*cs_numcomps],
                                        components[i*cs_numcomps + 1],
...
{
  OPLOGCALL("ctx /*%p*/, <gradient>, CGPointMake(%g, %g), CGPointMake(%g, %g), %d",
            ctx,
            startPoint.x, startPoint.y, endPoint.x, endPoint.y, options)
  cairo_pattern_t *pat = cairo_pattern_create_linear(startPoint.x, startPoint.y,
  ...
  CGGradientDrawingOptions options)
{
  ...
  cairo_pattern_t *pat = cairo_pattern_create_radial(startCenter.x,
...
void CGContextSetFont(CGContextRef ctx, CGFontRef font)
{
  ...
  cairo_set_font_face(ctx->ct,
                      cairo_scaled_font_get_font_face(((CairoFont*)font)->cairofont));
...
void CGContextShowText(CGContextRef ctx, const char *string, size_t length)
{
  ...
  cairo_matrix_init(&opaltextmatrix, ctx->txtmatrix.a, ctx->txtmatrix.b,
                    ctx->txtmatrix.c,
  ...
  for (int i=0; i<count; i++)
    {
      ...
      CGPoint userSpacePoint = CGPointApplyAffineTransform(positions[i],
                               CGContextGetTextMatrix(ctx));
  ...
  size_t count)
{
  ...
  for (size_t i=1; i<count; i++)
    {
      CGSize textSpaceAdvance = CGSizeApplyAffineTransform(advances[i-1],
                                CGAffineTransformInvert(CGContextGetTextMatrix(ctx)));
...
CGAffineTransform CGContextGetUserSpaceToDeviceSpaceTransform(CGContextRef ctx)
{
  ...
  return CGAffineTransformMake(cmat.xx, cmat.yx, cmat.xy, cmat.yy, cmat.x0,
                               cmat.y0);
}
