...
void dumpFontDescriptorInfo(CTFontDescriptorRef descriptor)
{
  ...
  NSLog(@"Attribute dictionary: %@", [CTFontDescriptorCopyAttributes(
  ...
  while (attrib = [attribEnumerator nextObject])
    {
      NSLog(@"Value for '%@': %@", attrib, [CTFontDescriptorCopyAttribute(descriptor,
                                            (CFStringRef)attrib) autorelease]);
    }
}
...
void dumpFontInfo(CTFontRef font)
{
  ...
  for (CGGlyph i=0; i<glyphs; i++)
    {
      CGSize advances;
      double advance = CTFontGetAdvancesForGlyphs(font, kCTFontDefaultOrientation,
                       &i, &advances, 1);
      NSLog(@"CTFontGetAdvancesForGlyphs %d returned %lf, and gave advance (%lf, %lf)",
            i, advance, (double)advances.width, (double)advances.height);

      CGRect rects;
      CGRect rect = CTFontGetBoundingRectsForGlyphs(font, kCTFontDefaultOrientation,
                    &i, &rects, 1);
      NSLog(@"CTFontGetBoundingRectsForGlyphs %d returned ((%lf,%lf),(%lf,%lf)), and gave rect ((%lf,%lf),(%lf,%lf))",
            i,
            (double)rect.origin.x, (double)rect.origin.y, (double)rect.size.width,
            (double)rect.size.height,
            (double)rects.origin.x, (double)rects.origin.y, (double)rects.size.width,
            (double)rects.size.height);

      CGSize translations;
      CTFontGetVerticalTranslationsForGlyphs(font, &i, &translations, 1);
      NSLog(@"CTFontGetVerticalTranslationsForGlyphs %d gave translation (%lf, %lf)",
            i, advance, (double)translations.width, (double)translations.height);
  ...
  NSLog(@"CTFontGetGlyphWithName('a'): %d", CTFontGetGlyphWithName(font,
        (CFStringRef)@"a"));
  ...
  NSLog(@"CTFontGetBoundingBox ((%lf,%lf),(%lf,%lf))", (double)bbox.origin.x,
        (double)bbox.origin.y, (double)bbox.size.width, (double)bbox.size.height);

  NSLog(@"CTFontGetUnderlinePosition %lf",
        (double)CTFontGetUnderlinePosition(font));

  NSLog(@"CTFontGetUnderlineThickness %lf",
        (double)CTFontGetUnderlineThickness(font));
  ...
  NSLog(@"CTFontGetMatrix %lf %lf %lf %lf %lf %lf", (double)xform.a,
        (double)xform.b, (double)xform.c, (double)xform.d, (double)xform.tx,
        (double)xform.ty);

  NSLog(@"CTFontGetSymbolicTraits %d", CTFontGetSymbolicTraits(font));

  NSLog(@"CTFontCopyAvailableTables: ");
  NSArray *availableTables = [CTFontCopyAvailableTables(font,
                              kCTFontTableOptionNoOptions) autorelease];
  ...
  NSLog(@"CTFontCopyFeatureSettings %@",  [CTFontCopyFeatureSettings(
          font) autorelease]);

  NSLog(@"CTFontCopyFontDescriptor %@",  [CTFontCopyFontDescriptor(
      font) autorelease]);
  ...
  NSLog(@"CTFontCopyVariationAxes %@", [CTFontCopyVariationAxes(
                                          font) autorelease]);
  ...
  NSLog(@"CTFontCopyCharacterSet %@", [CTFontCopyCharacterSet(
                                         font) autorelease]);

  NSLog(@"CTFontCopySupportedLanguages %@", [CTFontCopySupportedLanguages(
          font) autorelease]);
  ...
  while (nameKey = [enumerator nextObject])
    {
      NSLog(@"Name for '%@': %@", nameKey, [CTFontCopyName(font,
                                            nameKey) autorelease]);
    }
  ...
  while (nameKey = [enumerator nextObject])
    {
      NSString *lang = nil;
      NSString *localName = [CTFontCopyLocalizedName(font, nameKey,
                                                           &lang) autorelease];
      NSLog(@"Localized name for '%@': %@ (lang=%@)", nameKey, localName, lang);
    }

  NSLog(@"CTFontCopyPostScriptName %@", [CTFontCopyPostScriptName(
      font) autorelease]);
...
void enumerateBySerifStyle()
{
  ...
  CTFontDescriptorRef dummyDescriptor
    = [CTFontDescriptorCreateWithAttributes([NSDictionary dictionary])
       autorelease];
  NSArray *allDescriptors = [CTFontDescriptorCreateMatchingFontDescriptors(
                               dummyDescriptor, nil) autorelease];
  ...
  while (descriptor = [enumerator nextObject])
    {
      CTFontSymbolicTraits t = [[[descriptor objectForKey: kCTFontTraitsAttribute]
                                 objectForKey: kCTFontSymbolicTrait] intValue];
...
{
  CTFontDescriptorRef desc = [CTFontDescriptorCreateWithAttributes([NSDictionary
   ...
   */
  NSLog(@"Times initial descriptor URL: %@", [desc objectForKey:
             kCTFontURLAttribute]);


  CTFontRef font = CTFontCreateWithFontDescriptor(desc, 0, NULL);

  NSLog(@"Attributes after creating Times font: %@", [[font fontDescriptor]
        fontAttributes]);
  /**
   * Note this should print the URL even though it's not in the dictionary
   * printed on the previous line
   */
  NSLog(@"Times font URL: %@", [CTFontCopyAttribute(font,
                                kCTFontURLAttribute) autorelease]);

  CTFontDescriptorRef monoDesc
    = [CTFontDescriptorCreateWithAttributes([NSDictionary
        dictionaryWithObjectsAndKeys:
  ...
  NSLog(@"Monospaced font attributes: %@ URL: %@", [monoDesc
        fontAttributes], [monoDesc objectForKey: kCTFontURLAttribute]);

  dumpFontDescriptorInfo(monoDesc);

  CTFontDescriptorRef monoDescMatching
    = [CTFontDescriptorCreateMatchingFontDescriptor(monoDesc, [NSSet setWithObject:
                                                               kCTFontTraitsAttribute]) autorelease];
  NSLog(@"Monospaced font attribute after matching: %@ URL: %@", [monoDescMatching
        fontAttributes], [monoDescMatching objectForKey: kCTFontURLAttribute]);

  NSLog(@"Family name of font created with monoDesc: %@", [[CTFontCreateWithFontDescriptor(
          monoDesc, 12, NULL) autorelease] familyName]);
  NSLog(@"Family name of font created with monoDescMatching: %@", [[CTFontCreateWithFontDescriptor(
          monoDescMatching, 12, NULL) autorelease] familyName]);

  NSArray *monospaceFonts = [CTFontDescriptorCreateMatchingFontDescriptors(
                               monoDesc, [NSSet setWithObject: kCTFontTraitsAttribute]) autorelease];
  ...
  CGPathAddRect(path, NULL, CGRectMake(rect.origin.x, rect.origin.y + 100,
                                       rect.size.width/2, rect.size.height - 200));
  CGContextAddPath(ctx, path);
  CGContextSetRGBStrokeColor(ctx, 0, 0, 0, 1);
  CGContextStrokePath(ctx);

  NSAttributedString *as = [[[NSMutableAttributedString alloc] initWithString:
                                                                hamlet] autorelease];
  CTFramesetterRef fs = [CTFramesetterCreateWithAttributedString(
                           as) autorelease];

  CTFrameRef frame = [CTFramesetterCreateFrame(fs, CFRangeMake(0,0), path,
                                                   NULL) autorelease];
