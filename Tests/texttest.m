...
void dumpFontInfo(CGFontRef f)
{
  ...
  printf("CGFontGetFontBBox: ((%f,%f),(%f,%f))\n", (float)r.origin.x,
         (float)r.origin.y, (float)r.size.width, (float)r.size.height);
...
void draw(CGContextRef ctx, CGRect rect)
{
  ...
  CGContextSetTextMatrix(ctx,
                         xform); // Shows that the text position is part of the text matrix
  CGContextShowGlyphs(ctx, AEligatures, 2);

  xform = CGAffineTransformIdentity;
  xform = CGAffineTransformScale(xform, 2,
                                 2); // Font size is 20, scaling by 2, means 40 pt text
  xform = CGAffineTransformTranslate(xform, 15, 60);
  CGContextSetTextMatrix(ctx, xform);
  CGContextSetFontSize(ctx,
                       20); // This line should do nothing because the text matrix and font size are independent
  CGContextShowGlyphs(ctx, AEligatures, 2);
